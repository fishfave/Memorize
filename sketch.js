...
// Shared text size for Reading + Memorize passage text
let readingTextSize = 20;
...
let textArea, backButtonM, wordArea;
...
// Background water/bubble/light-ray effect
let bubbleCount;
let bubbles = [];
...
function switchPage(newState) {
  if (newState === "Home") {
    ...
    //  rect(28, 74, width - 56, height - 74 - 74, 14); text card
    ...
    buttons = [
      ...
      new Button(
        ...
        "+",
        () => {
          readingTextSize = min(readingTextSize + 2, MAX_TEXT_SIZE);
        },
        {
          textSize: 14,
          bgColor: [0, 0, 0, 0],
      ...
      new Button(
        ...
        "-",
        () => {
          readingTextSize = max(readingTextSize - 2, MIN_TEXT_SIZE);
        },
        {
          textSize: 14,
          bgColor: [0, 0, 0, 0],
...
function buildHomeLayout() {
  ...
  homeLayout = {
    ...
    navX: marginX,
    navW: cardW,
    toggle: { x: width / 2 - toggleW / 2, y: height -24 - toggleH, w: toggleW, h: toggleH },
  };
}
...
function createWordButtons() {
  buttons = [
    new Button(
     ...
     new Button(
      readButtonM.x + readButtonM.w / 2,
      readButtonM.y + readButtonM.h / 2,
      readButtonM.w,
      readButtonM.h,
      "Read",
      () => {
...
function drawMemorize() {
  ...
  // Draw the words that have been correctly selected so far, in a clipped box
...
function updateLayout(list) {
  ...
  memorizeScrollY = min(0, visibleHeight - contentHeight);
}

function clickMemorize() {}

//=====================================================================
//                   Favorites Page & Verse Storage
//=====================================================================
...
function mousePressed() {
  ...
  if (state === "Memorize") offsetY = memorizeScrollY - mouseY;
}
...
function dragUpdate() {
  if (state === "Reading") verseY = mouseY + offsetY;
  if (state === "Memorize") memorizeScrollY = mouseY + offsetY;
}
